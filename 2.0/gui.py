...
config(title="FlashScore 2.0", css_style=css)
        ...
        while True:
            ...
            data = input_group(
                inputs=[
                    ...
                    # Поле коэффициента
                    input(label="Коэффициент Тотал Больше 2.5", 
                        type=FLOAT, 
                        name="coefficient",
                        validate=check_field,
                        placeholder="1.50 - 2.50",
                        help_text="Минимальное значение коэффициента",
                        datalist=['1.50', '1.75', '2.00', '2.25', '2.50'],
                        required=True
                    )
                ]
            )
            ...
            with use_scope('scope1'):
                ...
                for id in search_list:
                    ...
                    if (k1_goal_home_sum != 0
                        ...
                        if float(points) > coefficient:
                            ...
                            rating_badge = f'<span style="background-color: {rating_color}; color: white; padding: 3px 8px; border-radius: 4px;">{round(points, 2)}</span>'
                ...
                put_success("Поиск завершен!").style(
                    'margin: 15px 0; '
                    'padding: 12px; '
                    'border-radius: 4px;'
                )
                    ...
                    with use_scope('scope1'):
                        ...
                        put_table(
                            ...
                            header=[
                                ...
                                put_html('<div style="background-color: #009879; color: white; padding: 12px 15px;">Рейтинг</div>')
                            ],
                            scope='scope1'
                        ).style(
                            'width: 100%; '
